...
if ($action === 'unlike') {
    $stmt = $conn->prepare("UPDATE plays SET likes = GREATEST(0, likes - 1) WHERE id = ?");
} else {
    $stmt = $conn->prepare("UPDATE plays SET likes = likes + 1 WHERE id = ?");
}
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->close();

$stmt2 = $conn->prepare("SELECT likes FROM plays WHERE id = ?");
